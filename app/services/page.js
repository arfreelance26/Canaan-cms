...
import api from "../api";
import { Plus, Trash2, Edit, X, Check, Layers } from "lucide-react";
...
export default function ServicesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedFile) {
      ...
      await api.post(`/services/${savedItem.id}/image`, fd);
    }
    ...
    <section className="relative bg-[#f5f4f0] font-sans flex flex-col p-4 sm:p-5 gap-3 min-h-screen">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-white px-6 py-8 sm:px-10 sm:py-10 flex items-end justify-between">
        ...
        <div className="mt-8">
          <h1 className="text-3xl sm:text-[2.6rem] font-bold tracking-[-0.03em] leading-[1.1]">
            Our
              <span style={{ color: "#85660c" }} className="italic font-normal"> Services</span>
          </h1>
        </div>
        <button
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl bg-white flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-[10px] font-medium tracking-[0.12em] uppercase text-neutral-300">No services yet</p>
          <p className="text-sm text-neutral-400">Add your first service offering above</p>
        </div>
      ) : (
          ...
          {items.map((item, i) => (
            <div key={item.id} className="relative rounded-2xl overflow-hidden bg-white flex flex-col group">
              ...
              {item.image_url ? (
                ...
                >
                  <Layers size={32} style={{ color: "rgba(133,102,12,0.35)" }} />
                </div>
              )}
