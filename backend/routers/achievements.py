...
@router.post("/{achievement_id}/image")
async def upload_achievement_image(achievement_id: int, file: UploadFile = File(...), db: Session = Depends(get_db), current_user: models.AdminUser = Depends(auth.get_current_user)):
    ...
    db_achievement.image_blob = contents
    db.commit()
    return {"ok": True}

@router.get("/{achievement_id}/image")
def get_achievement_image(achievement_id: int, db: Session = Depends(get_db)):
    db_achievement = db.query(models.Achievement).filter(models.Achievement.id == achievement_id).first()
    if not db_achievement or not db_achievement.image_blob:
        raise HTTPException(status_code=404, detail="Image not found")
    return Response(content=db_achievement.image_blob, media_type="image/jpeg")
