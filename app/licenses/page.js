...
import api from "../api";
import { Plus, Trash2, Edit, X, Check, ShieldCheck } from "lucide-react";
...
export default function LicensesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      ...
      const res = await api.post("/licenses/", formData);
      savedItem = res.data;
    }
    if (selectedFile) {
      const fd = new FormData();
      fd.append("file", selectedFile);
      await api.post(`/licenses/${savedItem.id}/image`, fd);
    }
    ...
    <section className="relative bg-[#f5f4f0] font-sans flex flex-col p-4 sm:p-5 gap-3 min-h-screen">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-white px-6 py-8 sm:px-10 sm:py-10 flex items-end justify-between">
        ...
        <div className="absolute top-0 right-0 bg-[#f5f4f0] px-5 py-3 rounded-bl-2xl flex items-center gap-2">
          ...
          <span className="text-[11px] font-medium text-neutral-900 tracking-tight">
            {items.length} licenses
          </span>
        </div>
        <div className="mt-8">
          <h1 className="text-3xl sm:text-[2.6rem] font-bold tracking-[-0.03em] leading-[1.1]">
            Licenses &<br />
            <span style={{ color: "#85660c" }} className="italic font-normal">Certifications</span>
          </h1>
        </div>
        <button
      ...
      {showForm && (
        <div className="relative rounded-2xl overflow-hidden bg-white">
          <div className="absolute top-0 left-0 bg-[#f5f4f0] px-5 py-3 rounded-br-2xl">
            <span className="text-[10px] font-medium tracking-[0.12em] uppercase text-neutral-400">
              {editingId ? "Editing license" : "New license"}
            </span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-7 pt-14 pb-7 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                required
                rows={3}
                placeholder="Details about this license or certification..."
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl bg-white flex flex-col items-center justify-center py-20 gap-3">
          <ShieldCheck size={28} className="text-neutral-200" />
          <p className="text-[10px] font-medium tracking-[0.12em] uppercase text-neutral-300">No licenses yet</p>
          <p className="text-sm text-neutral-400">Add your first license or certification above</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {items.map((item, i) => (
            <div key={item.id} className="relative rounded-2xl overflow-hidden bg-white flex flex-col group">
              ...
              {/* License image */}
              {item.image_url ? (
                <div className="relative h-48 mt-10 mx-4 rounded-xl overflow-hidden bg-neutral-100">
                  <img
                    src={item.image_url}
                    alt={item.title}
                ...
                <div
                  className="h-48 mt-10 mx-4 rounded-xl flex items-center justify-center"
                  style={{ background: "rgba(133,102,12,0.06)", border: "1px dashed rgba(133,102,12,0.2)" }}
                >
                  <ShieldCheck size={32} style={{ color: "rgba(133,102,12,0.35)" }} />
                </div>
              )}
              ...
              <div className="px-5 pb-5">
                <div
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[11px] font-semibold tracking-[0.08em] uppercase"
